...
public class UnitList {
     ...
     * @throws IllegalStateException - throw an IllegalStateException if it can't find a matching unit
     *         as that indicates there is a misalignment of the executing state and the complete list of possible units.
     */
    public Unit getUnit(String title, Character unitId) throws IllegalStateException {
        ...
        throw new IllegalStateException("Unit with subject title '" + title + "' and unit ID '" + unitId + "' not found");
    }

    /**
     * Creates a new `List` holding `references` to all the `Unit`s managed by the `UnitList` and returns it.
     *
    ...
    public String getFullDetail() {
        StringBuilder result = new StringBuilder("Unit List - " + units.size() + " unit(s)\n");
